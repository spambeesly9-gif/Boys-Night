...
const WINNER_LINES = [
  "Statistically, one of you peaked tonight.",
  "Your friends are funnier than you thought. Or you're worse.",
  "Someone go home and think about what they said.",
];
    ...
    <div className="min-h-screen bg-cream flex flex-col items-center justify-center px-4 py-12">
      ...
      <div className="w-full max-w-md text-center">
        <div className="mb-8">
          ...
          {isWinner ? (
            <p className="font-display text-xl italic text-brand-red mt-1">That's you. Genuinely surprised.</p>
          ) : (
            <p className="font-display text-xl italic text-gray-500 mt-1">{WINNER_LINES[Math.floor(Math.random() * WINNER_LINES.length)]}</p>
          )}
        </div>
            ...
            return (
              ...
              >
                ...
                <span className={`font-body font-bold flex-1 text-lg ${i === 0 ? 'text-amber-700' : isMe ? 'text-brand-red' : 'text-gray-800'}`}>
                  {p.name}
                  {isMe && <span className="text-xs ml-2 bg-brand-red/10 text-brand-red px-2 py-0.5 rounded-full">you</span>}
                </span>
                <span className={`font-display font-bold text-xl italic ${i === 0 ? 'text-amber-700' : 'text-gray-900'}`}>
